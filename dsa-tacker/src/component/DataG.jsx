...
function DataG(){
    ...
    const init = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/admin/graph`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
            setQ(response.data.graph);
        } catch (error) {
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <Card style={{ width: "100%", minWidth: "600px", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px" ,backgroundColor:"royalblue"}}>
...
function Each({ que }) {
    ...
    const boo=que.state;
        ...
        <Card style={{ width: "100%", minWidth: "600px", display: "flex", justifyContent: "space-between",marginTop:"15px" ,alignItems: "center", padding: "20px",backgroundColor:"lavender" }}>
            {/* <div>
            <Checkbox {...label} checked={boo} color="secondary" onClick={()=>{
                axios.get(`http://localhost:3000/admin/array`, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                });

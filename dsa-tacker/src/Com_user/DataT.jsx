...
function DataT() {
    ...
    const init = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/users/tree`, {
                headers: {
    ...
    const updateQuestionState = async (questionId) => {
        try {
            await axios.put(`http://localhost:3000/users/tree/${questionId}`, null, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <h2 style={{ fontSize: "35px", fontWeight: "lighter" }}>/Tree</h2>
            </div>
            <div style={{
                display: "flex",
...
function Each({ que, updateQuestionState }) {
    ...
    return (
        <Card style={{ width: "100%", minWidth: "600px", display: "flex", justifyContent: "space-between", marginTop: "15px", alignItems: "center", padding: "20px" ,backgroundColor:"l" }}>
            <div>

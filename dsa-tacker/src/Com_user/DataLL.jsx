...
function DataLL() {
    ...
    const init = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/users/ll`, {
                headers: {
    ...
    const updateQuestionState = async (questionId) => {
        try {
            await axios.put(`http://localhost:3000/users/ll/${questionId}`, null, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
...
function Each({ que, updateQuestionState }) {
    ...
    return (
        <Card style={{ width: "100%", minWidth: "600px", display: "flex", justifyContent: "space-between", marginTop: "15px", alignItems: "center", padding: "20px",backgroundColor:"lightcyan"  }}>
            <div>
